...
def plot_losses(train_losses, val_losses):
    ...
    plt.plot(range(1, len(val_losses) + 1), val_losses, label = 'train')
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.grid(True)
    plt.tight_layout()
...
def train_epoch(model, optimizer, criterion, loader, device, pad_ind):
    ...
    for source, target, source_len, target_len in tqdm(loader, desc='batch:'):
...
@torch.no_grad()
def val_epoch(model, optimizer, criterion, loader, device):
    model.eval()
    epoch_loss = 0.0

    for source, target, source_len, target_len in tqdm(loader, desc='batch:', position=0, leave=False):
        source = source.to(device)
        target = target.to(device)
        source_len = source_len.to(device)
        target_len = target_len.to(device)
        ...
        target_mask = torch.arange(seq_len, device=device)[None, :] < (target_len - 1)[:, None]

        logits = model(source, target_input, source_len, target_mask)
...
def train(model, optimizer, train_loader, val_loader, num_epochs, device, pad_ind):
    ...
    for epochs in tqdm(range(1, num_epochs + 1), desc='Epoch: ', position=1):
        train_loss = train_epoch(model, optimizer, criterion, train_loader, device, pad_ind)
        train_losses.append(train_loss)
        val_loss = val_epoch(model, optimizer, criterion, train_loader, device)
